...
flowchart TD
    A["EV 能源仓 (2048V) × 4A"] --> B
    subgraph B ["参数计算"]
        B1["availableEUt = 2048 × 4 = 8192"]
        B2["machineVoltage = 2048 (EV)"]
        B3["machineAmperage = 4"]
        B4["amperageOC = true"]
    end
    B --> C
    subgraph C ["升压检查"]
        C1["① 功率检查: 8192 ≥ 8192 ✓<br/>(总功率满足 IV 配方需求)"]
        C2["② 跨级检查: 8192 ≤ 2048 × 4^1 ✓<br/>(maxTierSkips=1 允许高 1 级)"]
        C3["③ 超频: log4(8192/8192) = 0<br/>(功率刚好够，无超频)"]
    end
    C --> D["消耗 8192 EU/t (4A EV)，原始速度运行 IV 配方"]
```
